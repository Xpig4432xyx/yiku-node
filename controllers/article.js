...
class ArticleController {
  ...
  static async createArticle(ctx, next) {
    const {title, author, desc, keyword, content, img_url, origin, publish, type, tags} = ctx.request.body;
    const createTime = new Date();
    const lastEditTime = new Date();
    if (!title || !author || !desc || !keyword || !content || !img_url || !type || !tags) {
      ctx.throw(400, '信息填写不完整');
    }
